...
const DEBUG_BAR_KEY = "opencode.debugBar.visible"

function readDebugBarVisible(): boolean {
  try {
    const raw = localStorage.getItem(DEBUG_BAR_KEY)
...
function writeDebugBarVisible(value: boolean) {
  try {
    localStorage.setItem(DEBUG_BAR_KEY, String(value))
  } catch {
    // ignore
  }
}
...
export function DebugBar() {
  ...
  const [visible, setVisible] = createSignal(readDebugBarVisible())

  const toggle = () => {
    const next = !visible()
    setVisible(next)
    writeDebugBarVisible(next)
  }
  ...
  return (
    <>
      <div class="pointer-events-auto fixed bottom-3 left-3 z-50 sm:bottom-4 sm:left-4">
        <Tooltip placement="top" value={visible() ? "Hide debug bar" : "Show debug bar"}>
          <IconButton
            icon={visible() ? "close-small" : "console"}
            variant="ghost"
            size="small"
            class="opacity-50 hover:opacity-100 transition-opacity"
            onClick={toggle}
            aria-label={visible() ? "Hide debug bar" : "Show debug bar"}
          />
        </Tooltip>
      </div>
      {visible() && (
