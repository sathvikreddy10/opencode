...
export const workspaceHandlers = HttpApiBuilder.group(WorkspaceApi, "workspace", (handlers) =>
  Effect.gen(function* () {
    ...
    const status = Effect.fn("WorkspaceHttpApi.status")(function* () {
      const ids = new Set((yield* svc.list((yield* InstanceState.context).project)).map((item) => item.id))
      return (yield* svc.status()).filter((item) => ids.has(item.workspaceID))
    })
...
).pipe(Layer.provide(Workspace.defaultLayer))
